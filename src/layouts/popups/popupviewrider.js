...
export default function ViewPopup(props) {
  ...
  const handleApproval = async (status) => {
    await http
      .post(`/v1/admin/approveOrReject${User}`, {
        [userIdKey]: props._id,
        status,
      }, {
        headers: {
  ...
  const ver = (props ?.status === "approved") ? (
      <MDBox ml={-1}>
      ...
      <div>
        ...
        <div style={{ textAlign: "right" }}>
          <MDBox ml={-1}>
            <MDButton onClick={()=>{handleApproval("approved"); close();}} style={{ padding: "0" }} variant="text" color="success"><MDBadge badgeContent="&#10004; APPROVE" color="success" variant="gradient" size="sm" /></MDButton>
            <MDButton onClick={()=>{handleApproval("rejected"); close();}} style={{ padding: "0" }} variant="text" color="error"><MDBadge badgeContent="&times; REJECT" color="error" variant="gradient" size="sm" /></MDButton>
          </MDBox>
        </div>
      </div>
    ) : (
        ...
        <div className="modal" style={{ maxWidth: "90vh", minWidth: "400px", height: "65vh", maxHeight: "65vh", overflow: "auto", boxShadow: "10px 10px 10px #9E9E9E", backgroundColor: "white", border: "1px solid gray", borderRadius: "20px", padding: "15px" }}>
          ...
          <div style={{ display: "flex" }}>
            <div style={{ textAlign: "center", maxWidth: "180px" }}>
              ...
              <img style={{ maxHeight: "180px", maxWidth: "100%" }} src={`${props.adhaarFile}`} />
            </div>
            <div style={{padding:"10px"}}>
              <div className="content"> <span style={{fontWeight:"bold"}}>PHONE</span> {props.phone} </div>
              <div className="content"> <span style={{fontWeight:"bold"}}>EMAIL</span> {props.email} </div>
              <div className="content"> <span style={{fontWeight:"bold"}}>AADHAAR CARD NO.</span> {props.adhaarCardNumber} </div>
            </div>
          </div>
          <div>
          ...
          <div style={{ position: "absolute", bottom: "0", padding: "20px", width: "90%", boxSizing: "border-box" }}>
          {ver}
        </div>
        </div>
        )
